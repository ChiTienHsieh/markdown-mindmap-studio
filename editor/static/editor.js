...
async function loadLocale() {
    try {
        ...
        console.warn('Could not load locale:', e);
    }
}

function applyLocale() {
    // Title
    document.title = locale.app?.title || 'Mindmap Editor';
    const headerTitle = document.getElementById('header-title');
    if (headerTitle) headerTitle.textContent = locale.header?.title || 'Mindmap Editor';

    // Tooltips
    const btnTheme = document.getElementById('btn-theme-toggle');
    if (btnTheme) btnTheme.title = locale.buttons?.themeToggle || '';
    ...
    if (agentInput) agentInput.placeholder = locale.agent?.inputPlaceholder || '';
    const agentSend = document.getElementById('agent-send');
    if (agentSend) agentSend.textContent = locale.agent?.sendButton || 'Send';

    // Agent welcome
    const welcomeEl = document.getElementById('agent-welcome');
    if (welcomeEl && locale.agent?.welcome) {
        const w = locale.agent.welcome;
        const caps = (w.capabilities || []).map(c => `<li>${c}</li>`).join('');
        welcomeEl.innerHTML = `<p>${w.intro || ''}</p><ul>${caps}</ul><p>${w.example || ''}</p>`;
    }
...
function handleAIAction(action) {
    ...
    if (!content.trim()) {
        elements.aiResponse.innerHTML = `<div class="ai-response-placeholder">${locale.ai?.selectTextFirst || 'Please select text first'}</div>`;
        return;
    }
...
function applyAISuggestion() {
    ...
    if (selectedText) {
        ...
        const aiSection = locale.ai?.suggestionSection || 'AI Suggestion';
        editor.value += `\n\n---\n\n## ${aiSection}\n\n` + state.aiResponse;
    }
...
async function copyAIResponse() {
    ...
    try {
        ...
        elements.aiCopy.textContent = locale.ai?.copySuccess || '✓ Copied';
        setTimeout(() => {
            elements.aiCopy.textContent = locale.ai?.copyButton || 'Copy';
        }, 2000);
    } catch (error) {
        console.error('Failed to copy:', error);
        alert(locale.ai?.copyFailed || 'Copy failed');
    }
}
